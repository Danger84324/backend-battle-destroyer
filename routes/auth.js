...
router.post('/signup', async (req, res) => {
  try {
    ...
    // ✅ Generate token immediately — no OTP step
    ...
    const responseData = {
      ...
      message: isReferralValid
        ? `Account created! You received +2 referral credits. Welcome!`
        : 'Account created successfully! Welcome to Battle Destroyer.',
      token,
      user: {
        userId: user.userId,
...
router.post('/login', async (req, res) => {
  try {
    ...
    // emailVerified check kept — handles old unverified accounts gracefully
    if (!user.emailVerified) {
      console.log(`[Login] Unverified email attempt: ${email}`);
      return sendEncryptedError(res, 401, 'Your account is not verified. Please contact support.');
    }
